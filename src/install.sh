...
if [ "${dev}" = true ]; then
  nfqws_options="--hostspell=hoSt --dpi-desync-badseq-increment=0 --dpi-desync-fooling=md5sig --dpi-desync-split-seqovl=1 --dpi-desync=fakeddisorder --dpi-desync-ttl=1 --dpi-desync-autottl=-5 --dpi-desync-split-pos=1"
else
